...
import {MonoglyphicFont, MontserratFont} from "@/app/font/font";
...
const Title = async ({children, animate, size}: TitleProps) => {
  const baseClasses = `w-max font-bold text-black dark:text-white text-center tracking-tighter drop-shadow-2xl tracking-wide`;
  ...
  return (
      <div className="w-max">
        {createElement(
            headingTag,
            {className: cn(baseClasses, fontSize, animation, MonoglyphicFont.className)},
            children
        )}
      </div>
  );
